...
char * settingText[numSettings]={"Rotate Screen = ","FFT Ref= ", "S-Meter Zero= ","SSB Gain EQ-H= ","SSB Gain EQ-M2= ","SSB Gain EQ-M1= ","SSB Gain EQ-L= ","SSB Rx Filter High= ","SSB Rx Filter Low= ", "Tx Gain= ","Rx Gain= ","SSB Mic Gain= ","FM Mic Gain= ","AM Mic Gain= ","Repeater Shift= ","CTCSS= "," Rx Offset= ","Rx Harmonic Mixing= "," Tx Offset= ","Tx Harmonic Mixing= ","Band Bits (Rx)= ","Band Bits (Tx)= ", "CW Ident= ", "CWID Carrier= ", "CW Break-In Hang Time= ", "24 Bands= "};
enum {ROTATE,FFT_REF,S_ZERO,SSB_GEQH,SSB_GEQM2,SSB_GEQM1,SSB_GEQL,SSB_FILT_HIGH,SSB_FILT_LOW,TX_GAIN,RX_GAIN,SSB_MIC,FM_MIC,AM_MIC,REP_SHIFT,CTCSS,RX_OFFSET,RX_HARMONIC,TX_OFFSET,TX_HARMONIC,BAND_BITS_RX,BAND_BITS_TX,CWID,CW_CARRIER,BREAK_IN_TIME,BANDS24};
    ...
    scaling = 255.0/(float)(fftref-baselevel);
        ...
        {  
          ...
          {                                                                                                           
            ...
            //scale to 0-255
            level = (buf[p][r]-baselevel+5)*scaling;   
        ...
        //draw spectrum bars
            drawLine(653, 130, 653, 365,128,128,128);
            drawLine(653, 365, 138, 365,128,128,128);
            drawLine(138, 365, 138, 130,128,128,128);
            drawLine(138, 130, 653, 130,128,128,128);//linha superior
            drawLine(138, 216, 653, 216,128,128,128);
            drawLine(654, 130, 654, 365,128,128,128);
            drawLine(653, 366, 138, 366,128,128,128);
            drawLine(137, 365, 137, 130,128,128,128);
            drawLine(138, 129, 653, 129,128,128,128);//linha superior													 
        ...
        {  
          ...
          switch(bandFFTBW[band])
          {
            ...
            break;
            case 1:                 //24KHz BW
			  FFTRef=FFTRef-2;      // compensar nivel de FFTRef 
...
void TempC(void)
{
		  ...
		  int numread = fread(vtemp,10,1,temp);
		  fclose(temp);
		  gotoXY(5,110);
		  setForeColour(255,255,0);
		  textSize=1;
		  displayStr("Temp CPU");
		  sprintf(vtemp,"=%5.1fC     ",strtol(vtemp,&end,10)/1000.0);
		  gotoXY(70,110);                                         
		  displayStr(vtemp);          
}
...
void setMode(int md)
{
  ...
  if(md==FM)
    {
    sendFifo("M4");    //FM
    setRxFilter(-12500,12500);    //FM Filter
    setTxFilter(-12500,12500);    //FM Filter  
    setFreq(freq);    //set the frequency to adjust for CW offset.
    ritButton(0);
    setRit(0);
    } 
...
void setHwRxFreq(double fr)
{
  ...
  if (rxfreqhz<28000000) rxfreqhz=2800000;         //this is the lowest frequency we can receive
  ...
  if(LOrxfreqhz!=lastLOhz)         
    {
      setLimeRxFreq(LOrxfreqhz); //Control Pluto directly to bypass problems with Gnu Radio Sink ???
      lastLOhz=LOrxfreqhz;
    }
...
{                                                                        
  ...
  setLimeTxFreq(txfreqhz); //Control Pluto directly to bypass problems with Gnu Radio Sink ???
}
...
//>>> STATUS de TX GAIN,RX GAIN,MIC_Gain e FM DEV
     ...
     textSize=1;                                         
	 displayStr("Tx Gain= "); 	 
     sprintf(vtxStr,"%d ",bandTxGain[band]);                                         
	 gotoXY(70,130);
     displayStr(vtxStr);
     ...
     textSize=1;
	 displayStr("Rx Gain= ");
     sprintf(vrxStr,"%d ",bandRxGain[band]);                                         
	 gotoXY(70,150);                                         
     displayStr(vrxStr);
...
void changeSetting(void)
{
    ...
    if(settingNo==FFT_REF)        // FFT Ref Level
      {
      FFTRef=bandFFTRef[band];   //melhor sitio para repor o FFTRef modo BW=0 e quando ajustar via menu nos modos BW=1,2,3 ṇo altera? parece que agora funciona ...
    ...
    if(settingNo==TX_GAIN)        // Tx Attenuator
      ...
      mouseScroll=0;
      if(bandTxGain[band]>80) bandTxGain[band]=80;
      if(bandTxGain[band]<-12) bandTxGain[band]=-12;
      setLimeTxGain(bandTxGain[band]);
      displaySetting(settingNo);  
      }  
     if(settingNo==RX_GAIN)        // Rx Gain Setting
      ...
      mouseScroll=0;
      if(bandRxGain[band]<-12) bandRxGain[band]=-12;
      if(bandRxGain[band]>61) bandRxGain[band]=61;
      setLimeRxGain(bandRxGain[band]);
      displaySetting(settingNo);  
      }
...
if(se==BAND_BITS_TX)
  ...
  if(se==RX_GAIN)
  {
  sprintf(valStr,"%d",bandRxGain[band]);
  displayStr(valStr);
  }
  if(se==SSB_GEQH)
  {
