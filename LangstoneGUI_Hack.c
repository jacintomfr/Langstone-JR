...
// ── Memory channels ──────────────────────────────────────────────────────────
// 12 memory channels — store freq (MHz) + mode + label
...
void displayMenu()
{
  ...
  // Button 5 — MEM (replaces SNAP — SNAP now via SET menu Button 4 in SETTINGS mode)
...
if(buttonTouched(funcButtonsX+buttonSpaceX*4,funcButtonsY))    //Button 5 = MEM (FREQ) / UPGRDE (SETTINGS)
    ...
    if(inputMode==SETTINGS)
      ...
      {
      // FREQ mode: open/close MEM popup
...
if(inputMode==SETTINGS)
  ...
  for(int cy = settingY-29; cy < settingY; cy++)
    drawLine(0, cy, 799, cy, 0,0,0);
  // Clear displayError area (errorX=200, errorY=240) — UPGRDE confirm msg residual
  gotoXY(0, errorY);
  textSize=2;
  displayStr("                                                ");
  gotoXY(0, errorY+8);
...
if(se==BAND_BITS_TX)
  ...
  {
  maxSetIndex = 10;
  // Clear BOTH pixel rows — textSize=2 spans settingY and settingY+8 (descenders go lower)
  for(int _cy = settingY-2; _cy < settingY+20; _cy++)
    drawLine(0, _cy, 799, _cy, 0, 0, 0);
  setForeColour(255,255,255);
  gotoXY(0, settingY);
  displayStr(settingText[CALLSIGN]);  // reprint "Call ID= "
  for(int c=0; c<11; c++)
    {
